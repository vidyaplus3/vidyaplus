import { db, auth } from './firebase-init.js';
        ...
        window.openVideo = (vidUrl, title, pdfUrl) => {
           ...
           // 🚨 HISTORY PUSHSTATE APPLIED HERE
            ...
            if (window.ytPlayer && typeof window.ytPlayer.destroy === 'function') {
                ...
                } // 🚨 FIX: Ye bracket miss ho gaya tha pehle!
            });
        };
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'F12' || (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'J' || e.key === 'C')) || (e.ctrlKey && e.key === 'U')) { e.preventDefault(); return false; }
                    ...
                    // ==========================================
        // 🚀 SMART PLAYER LOGIC: DRAG, SKIP & SPEED 
        // ==========================================

        // 1. Interval Update (Dragging ke time interference rokne ke liye)
        ...
        // 3. Settings Menu & Playback Speed Logic
        window.toggleSettings = (e) => { 
            e.stopPropagation(); 
            document.getElementById('settings-menu').classList.toggle('show'); 
        };
        
        // Agar Settings Menu ke bahar click ho toh usko band kar do
        ...
        // 4. Draggable Progress Bar (Scrubbing Magic)
        ...
        window.updateScrub = (e) => {
            ...
            // Support for both PC Mouse and Mobile Touch
        ...
        window.showUI = (e) => {
            // 🚨 GHOST TRACKER: Fake mousemove ko block karne ke liye
            if (e && e.type === 'mousemove') {
                if (e.clientX === lastMouseX && e.clientY === lastMouseY) return; // Agar mouse actually nahi hila, toh ignore karo
            ...
            if (state === 1) { 
                window.uiTimeout = setTimeout(() => {
                    const menu = document.getElementById('settings-menu');
                    if (menu && menu.classList.contains('show')) return; // Settings khuli hai toh hide mat karo
        ...
        // 🚨 SMART SHIELD LOGIC (UX FIX) 🚨
        window.handleShieldClick = () => {
            const controls = document.getElementById('custom-controls');
            if(controls && controls.classList.contains('hidden')) {
                // Agar controls chhupe hain, toh sirf unhe dikhao (video pause MAT karo)
                window.showUI();
            } else {
                // Agar controls pehle se dikh rahe hain, tab play/pause karo
                window.togglePlay();
            }
        };
