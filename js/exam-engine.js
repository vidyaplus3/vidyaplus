...
// 🚨 1. THE ANTI-GLITCH SHIELD
// Agar bachha galti se back button dabakar exam page pe aata hai, toh usko block karo!
if (sessionStorage.getItem('submitted_' + testId)) {
    document.getElementById('loader').innerHTML = `
        <div style="text-align:center; padding: 30px;">
            <i class="fas fa-shield-alt" style="color: #10b981; font-size: 3rem; margin-bottom:15px;"></i>
            <h3 style="color: #1e293b; margin-bottom: 10px; font-weight: 800;">Test Already Submitted</h3>
            <p style="color: #64748b; font-size: 0.9rem; margin-bottom: 25px;">You cannot navigate back to an active exam session using the browser's back button.</p>
            <button onclick="window.location.href='study.html#tests'" style="padding: 12px 25px; background: #2563eb; color: white; border: none; border-radius: 8px; font-weight: 700; cursor:pointer;">Go to Dashboard</button>
        </div>
    `;
    throw new Error("Execution halted: Session already submitted."); // Script yahin ruk jayegi, loader nahi fusega
}
...
onAuthStateChanged(auth, async (user) => {
    ...
    if(!testId || !batchId) { window.location.href = 'study.html#tests'; return; }
    ...
    try {
        ...
        if (testSnap.exists()) {
            ...
            if(!testData.questions || testData.questions.length === 0) {
                alert("This test has no questions."); window.location.href = 'study.html#tests'; return;
            }

            document.getElementById('loader').style.display = 'none';
            initializeTestUI();
        } else {
            alert("Test not found in database."); window.location.href = 'study.html#tests';
        }
    } catch (err) { 
...
async function autoSubmit() {
    ...
    document.getElementById('loader').innerHTML = `<div class="spinner"></div><h3 style="color: #1e293b; font-weight: 700;">Evaluating Answers...</h3>`;
    ...
    try {
        ...
        // 🔒 LOCK LAG GAYA: Test ho chuka hai
        sessionStorage.setItem('submitted_' + testData.docId, 'true'); 
        
        // Wapas dashboard phek do
        window.location.href = 'study.html#tests'; 
