...
export const CommentEngine = {
    ...
    renderUI: (containerElement, lectureId) => {
        ...
        // 🚨 NAYA WRAPPER: Taki Input hamesha bottom par chipka rahe
        containerElement.innerHTML = `
            <div class="chat-layout-wrapper" style="height: 100%; display: flex; flex-direction: column;">
                <div style="padding: 15px 20px; font-weight: 700; font-size: 1.1rem; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #e2e8f0; background: #fff;">
                ...
                <div id="comments-container" style="flex: 1; overflow-y: auto; padding: 20px;">
        ...
        setTimeout(() => {
            ...
            // 🚨 NAYA: Inverted Scroll logic (Scroll UP to load more)
            CommentEngine.setupInvertedScroll(); 
        }, 50);
    },

    // 2. HTML Generator (100% UNTOUCHED from your file)
    ...
    // 3. Fetch Data (Modified to Invert Comments Order)
    fetchComments: async (isLoadMore = false) => {
        ...
        try {
            ...
            if (result.comments.length === 0 && !isLoadMore) {
                ...
                // 🚨 NAYA: Reverse the array to put newest at bottom
                ...
                if (isLoadMore) {
                    ...
                    container.scrollTop = container.scrollHeight - oldScrollHeight; // Prevent jumping
                } else {
                    container.innerHTML = htmlToInsert;
                    container.scrollTop = container.scrollHeight; // Scroll to bottom on load
                }

                // Add replies AFTER inserting main comments
                sortedComments.forEach(comment => {
    ...
    // 4. Security, Logic & Interactive Clicks (100% UNTOUCHED)
    initSecurity: (lectureId) => {
        ...
        btn.addEventListener('click', async () => {
            ...
            // Send user to bottom to see their comment
    ...
    // 5. Zero-Lag Optimistic Render (Modified for Inverted Flow)
    postOptimistic: (text, parentId = null) => {
        ...
        if (parentId) {
            ...
            // 🚨 NAYA: Kyunki naye message bottom pe hain, `beforeend` lagega
    ...
    // 🚨 NAYA: Inverted Scroll Listener (Upar scroll karne pe purane aayenge)
    setupInvertedScroll: () => {
        const scrollArea = document.getElementById('comments-container'); 
