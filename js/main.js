...
window.renderSubjects = () => { /* Logic hidden for brevity, exact same as before */ };
window.renderChapters = (subjectName) => { /* Logic hidden for brevity, exact same as before */ };
window.filterClassroom = (filterType, btnElement = null) => { /* Logic hidden for brevity, exact same as before */ };
window.renderExtraMaterials = (containerId, items, emptyMsg) => { /* Logic hidden for brevity, exact same as before */ };
window.switchClassroomTab = (type) => { /* Logic hidden for brevity, exact same as before */ };

// ==========================================
// SECURE TEST PORTAL GATEWAY
// ==========================================
...
window.openInstructions = (testId) => {
    ...
    document.getElementById('inst-subject').innerText = test.subject || 'Standard Assessment';
...
window.startTestPlayer = () => {
    ...
    document.getElementById('instruction-mode').style.display = 'none'; // Hide Modal
    
    // Naya tab me NTA Portal open karega
...
const setupDropdown = (batchIds) => {
    ...
    batchIds.forEach(async (bId) => {
        ...
        if (bSnap.exists()) { dropdown.innerHTML += `<div class="dropdown-item" onclick="window.switchBatch('${bId}')">${bSnap.data().title}</div>`; }
    });
};
...
// Render classes UI hidden for brevity (subject, chapter logic is intact)
